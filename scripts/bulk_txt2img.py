...
def main():
    ...
    parser.add_argument(
        "--make_grid",
        action="store_true",
        help="save a grid, not only individual samples. Helpful when evaluating lots of samples",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="how many times to create the samples",
    )
    ...
    if not opt.from_file:
        ...
        datas = [[batch_size * [prompt]]]

    else:
        print(f"reading prompts from {opt.from_file}")
        with open(opt.from_file, "r") as f:
            prompt_list = f.read().splitlines()
            datas = [[batch_size * [i]] for i in prompt_list]
                ...
                for _ in range(opt.n_batches):
                    ...
                    with model.ema_scope():
                                ...
                                if not opt.skip_save:
                                    for x_sample in x_samples_ddim:
                                        ...
                                        Image.fromarray(x_sample.astype(np.uint8)).save(
                                            os.path.join(
                                                sample_path,
                                                f"{prompts[0].replace(' ', '_')}_s{seed:07}_{base_count:05}.png",
                                            )
                                        )
                                        base_count += 1
                        ...
                        if opt.make_grid:
                            ...
                            # to image
                            grid = (
                                255.0 * rearrange(grid, "c h w -> h w c").cpu().numpy()
                            )
